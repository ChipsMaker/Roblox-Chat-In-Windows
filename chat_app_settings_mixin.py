...
class ChatAppSettingsMixin:
    def load_settings(self):
        ...
        if os.path.exists(self.settings_path):
            ...
            if not signature or not hmac.compare_digest(signature, expected_sig):
                # Подпись не совпадает — значит, файл редактировали вручную
                # Сбрасываем защищённые поля до дефолта и перезаписываем файл
                self.settings["name"] = ""
                self.settings["last_changed"] = 0
                self.save_settings()
                return

            # Всё в порядке — добавляем отсутствующие ключи из default_settings
    ...
    def save_settings(self):
        # Добавляем поле signature перед записью
    ...
    def _get_or_create_signing_key(self):
        """Возвращает 32-байтовый ключ для HMAC. Если ключа нет — создаёт и сохраняет в реестре."""
        reg_path = r"Software\Red65\Chat"
        try:
            ...
            # Ключ реестра отсутствует — создаём
            key = winreg.CreateKey(winreg.HKEY_CURRENT_USER, reg_path)
            new_key = os.urandom(32)  # 256 бит
            winreg.SetValueEx(key, "SigningKey", 0, winreg.REG_BINARY, new_key)
            winreg.CloseKey(key)
            return new_key

    def _compute_signature(self, settings_dict):
        """Вычисляет HMAC-SHA256 от JSON-представления защищённых полей."""
        # Берём только поля, которые нужно защитить
        protected = {
            "name": settings_dict.get("name", ""),
            "last_changed": settings_dict.get("last_changed", 0)
        }
    ...
    # В файле chat_app_settings_mixin.py добавь этот метод:
    def show_settings_dialog(self):
        # Если твой основной метод называется иначе, просто вызови его здесь
        # Например, если он называется settings_window():
        if hasattr(self, 'open_settings'):
            self.open_settings()
        else:
            # Если метода еще нет, создадим заглушку
            QMessageBox.information(self, "Settings", "Ошибка открытия настроек.")

        def check_updates(self):
            """Проверка обновлений (перенесено из client.py)"""
            ...
            try:
                ...
                if latest_ver > self.VERSION:   # используем VERSION из класса ChatApp
                    m = QMessageBox(self)
